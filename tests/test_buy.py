...
@allure.description("Test buy product not available") #Второй тест для запуска, когда товара нет в наличии
def test_buy_product_not_available():
    options = webdriver.ChromeOptions()
    options.add_experimental_option("detach", True)
    options.add_argument("--guest")
    options.add_argument("--disable-notifications")
    ...
    llp = Led_lamps_page(driver)
    llp.choose_lamp_for_buy_if_not_available() # Измененный метод, ищет товары не только в наличии, но и под заказ
    llp.buy_found_product()
    # llp.choose_flux_filters()
    # llp.choose_type_base_and_glow_temp()
    cp = Cart_page(driver)
